...
\\
The three scenarios described in the next sections provides examples of how we envision the usage of the PeacefulBanana tool in three different settings. First as a individual tool on a daily basis, then as part of a team collaborative reflection session, and finally as a reflection session across teams - in order to learn from different projects. 
The two first scenarios were developed early in the development process. The first scenario features using the tool at the end of each working day as inspired by [ref to the 5 minute daily reflection session]. The second scenario where in the end of each iteration there is a retrospective reflection session. The third scenario shows usage of the tool as part of the mentioned retrospective workshop at the end of the course. The third scenario were developed a bit later, as we wanted to show how the tool can promote reflection across teams. 
\\
...
\label{scenario2}
Using the tool as part of the agile methodic two-week reflection sessions. 
This scenario uses the tool to indicate how the project has been over the last iteration, tightly coupled with milestones. Generate tagclouds based on the trending issues in the relevant milestones, activity graphs, mood trajectories etc. The tool will enable a group to see the whole project course more clearly, but also be able to dive into certain issues or milestones that showed to be of particular interest, which in turn can aid reflection. 
